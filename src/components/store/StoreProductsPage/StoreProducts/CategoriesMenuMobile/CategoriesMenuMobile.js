...
import styles from './CategoriesMenuMobile.module.scss';
import categoriesData from '../../categories';
import { FaChevronDown } from 'react-icons/fa';
import axios from 'axios';
import { apiK, apiP } from '../../../../../apiConfig';

function CategoriesMenuMobile(props) {
  const [activeCategory, setActiveCategory] = useState(null);
  const refs = useRef({});

  const [isMenuOpen, setIsMenuOpen] = useState(false);

  const [menuTitle, setMenuTitle] = useState('Rozwiń menu kategorii');

  const toggleMenu = () => {
    setIsMenuOpen(!isMenuOpen);

    if (isMenuOpen) {
      setMenuTitle('Rozwiń menu kategorii')
    } else {
      setMenuTitle('Zwiń menu kategorii')
    }
  };
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={isMenuOpen ? styles.storeProducts : styles.storeProductsClosed}>
